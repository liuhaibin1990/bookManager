...
#pragma mark SQL manipulation 判断obj的值类型，并绑定对应类型的值
...
//            该函数绑定double浮点数
        ...
        //比较两个字符串相等返回0 str1>str2 返回正数，str1<str2 返回负数 判断objc的数据类型
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (namedIdx > 0) {
                // Standard binding from here. 这判断类型绑定
    ...
    if (idx != queryCount) {//如果id不等于查询的参数序列证明错误
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    //没有缓存执行
    if (!pStmt) {
        //sqlite3的执行语句 参数说明，1 数据库，2 sql，3，unknow， 4 被预编译的语句 ,5 unknow
        rc = sqlite3_prepare_v2(_db, [sql UTF8String], -1, &pStmt, 0);
        
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                //输出最后一次调用的错误码和错误信息 sql语句 以及db
                NSLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                NSLog(@"DB Query: %@", sql);
                NSLog(@"DB Path: %@", _databasePath);
            }
